...
signif_mask_dv = 10 # b-strong=150 km/s; CF after masking practically same as dv=300
...
_, sig_pdf_fit_mask = reion_utils.pdf_calc(civ_tot.signif[civ_tot.signif_gpm], sig_min, sig_max, nbins)
...
# cgm only
start = time.time()
meanflux_cgm = np.mean(flux_cgm_lores)
deltaf_cgm = (flux_cgm_lores - meanflux_cgm) / meanflux_cgm
vel_mid, xi_cgm, npix_cgm, _ = reion_utils.compute_xi(deltaf_cgm, v_lores, vmin_corr, vmax_corr, dv_corr)
xi_mean_cgm = np.mean(xi_cgm, axis=0) # 2PCF from all the skewers
end = time.time()
print("............ CGM only: compute xi done in", (end-start)/60, "min")
"""
# igm + cgm
start = time.time()
...
#plt.plot(vel_mid, xi_mean_cgm/(scalefactor*50), linewidth=linewidth, linestyle='-', c='tab:red', label='CGM/50')
